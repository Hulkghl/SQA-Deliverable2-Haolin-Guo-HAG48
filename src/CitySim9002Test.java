...
public class CitySim9002Test 
{
	//1.test if the types of visitors stored the right type in the arrayList, each corresponding
		...
		//6. Stubbing test. Test if a visitor visit a place by setting the location name as "The SQA Class"
		//Setting the Generator's result number to 3 at the same time, the output result should be 
		//"Visitor 3 is going to The SQA Class!" as expected 
		@Test
		public void testVisiting()
		{
			...
			Mockito.when(cl.Location(3)).thenReturn("The SQA Class");
			FunVisitor v = new FunVisitor();
			v.n = 3;
			v.Visitortype = 3;
			assertEquals("Visitor 3 is going to The SQA Class!", v.Visiting(cl, pr, RG));
		}
